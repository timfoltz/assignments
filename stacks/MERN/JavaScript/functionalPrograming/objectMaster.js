...
const idArr = pokémon.filter( p => p.id % 3 ===0 );
// console.log(idArr);
const fireTypeArr = pokémon.filter( p => p.types.includes("fire"));
...
const flyingSecondPos = pokémon.filter(p => p.types[1] =="flying").map(p => p.types[0]);
console.log(flyingSecondPos);
